...
- `ElementSystem.applyElementEffect()` has bespoke status or hit logic for Fire, Water, Wind, Earth, and the legacy Lightning/Frost/Acid effects.
- Non-primary owned elements are still listed in `stats.activeElements` for HUD/combo detection and can apply their own hit behavior when implemented.

| Element | ID | Icon | Family | Identity | Brick Breaker Role | Run Behavior | Current Code Behavior | Fitting Implementable Behavior |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Fire | `element_fire` | `F` | Classic | Heat, burn, eruption | Damage over time, explosive clears | Adds burn effects, flame spread hooks, and fire combo access. | Adds `fire` to `stats.activeElements` and combo detection. If first owned, becomes primary `stats.element` for Fire visuals and weakness/resistance lookup. Fire hits can now apply the existing `burn` status even when `burnPower` is 0. | Implemented: boss-picked Fire grants intrinsic burn using the existing `burn` status for short damage-over-time ticks, with stack caps reduced on bosses. |
| Water | `element_water` | `W` | Classic | Flow, soak, spread | Chaining, cleansing, splash damage | Adds splash effects, wet marks, cleanse hooks, and water combo access. | Adds `water` to `stats.activeElements` and combo detection. If first owned, gives Water visuals and weakness/resistance lookup. Water hits now splash 35% final damage to up to two nearby active targets, spending the secondary hit budget. | Implemented: Water hits splash 35% of final damage to the nearest one or two active targets in a small radius, spending the existing secondary hit budget. |
| Wind | `element_wind` | `N` | Classic | Motion, lift, direction | Ball speed, curve, multiball nudges | Adds gust effects, directional nudges, curve hooks, and wind combo access. | Adds `wind` to `stats.activeElements` and combo detection. If first owned, gives Wind visuals and weakness/resistance lookup. Ball hits now get a small post-hit speed impulse and steering toward a nearby active target; cannon projectile movement is unchanged. | Implemented: Wind adds a small post-hit speed impulse and bends the reflected ball velocity toward the nearest live target, still clamped by current speed caps. |
| Earth | `element_earth` | `E` | Classic | Stone, weight, armor | Durability, shields, impact damage | Adds heavy impacts, tremor hooks, defensive hooks, and earth combo access. | Adds `earth` to `stats.activeElements` and combo detection. If first owned, gives Earth visuals, weakness/resistance lookup, and `baseDamageMultiplier: 1.02`. Earth hits now tremor-chip nearby bricks/enemies for 25% final damage, or add a small bonus hit against bosses. | Implemented: Earth hits create a tremor that chips nearby bricks/enemies for 25% of final damage; against bosses, the tremor becomes a small direct bonus hit. |
| Spark | `element_spark` | `K` | Arcade Physics | Energy, charge, chain reactions | Arcs, speed, conductive hits | Adds chain arcs, charge marks, speed hooks, and spark combo access. | Adds `spark` to `stats.activeElements` and combo detection. If first owned, gives Spark visuals, weakness/resistance lookup, and `baseDamageMultiplier: 0.98`. No arc/chain/static code yet; legacy chain code is for `lightning`, not `spark`. | Spark should reuse the existing chain-target pattern: one or two short arcs jump to nearby targets for reduced damage, balanced by its lower base damage multiplier. |
| Resin | `element_resin` | `R` | Arcade Physics | Sticky growth, binding | Slows, traps, coats bricks | Adds sticky coatings, linked-brick hooks, slow zones, and resin combo access. | Adds `resin` to `stats.activeElements` and combo detection. If first owned, gives Resin visuals and weakness/resistance lookup. No sticky, slow, coating, or linked-brick code yet. | Resin should apply a sticky `resin` status; later hits on coated targets deal modest bonus damage and leak a small share to one nearby coated or adjacent target. |
| Echo | `element_echo` | `O` | Arcade Physics | Sound, vibration, repetition | Delayed hits, pulse waves | Adds delayed repeat-hit hooks, pulse waves, and echo combo access. | Adds `echo` to `stats.activeElements` and combo detection. If first owned, gives Echo visuals and weakness/resistance lookup. No repeat-hit or pulse code yet. | Echo should queue one delayed repeat hit for about 40% damage after a short pause; if the original target is gone, it can retarget the nearest active target in range. |
| Gravity | `element_gravity` | `G` | Arcade Physics | Pull, mass, orbit | Curve shots, wells, compression | Adds gravity wells, pull effects, orbit hooks, and gravity combo access. | Adds `gravity` to `stats.activeElements` and combo detection. If first owned, gives Gravity visuals, weakness/resistance lookup, and `baseDamageMultiplier: 1.01`. No pull, curve, orbit, or compression code yet. | Gravity should pull the ball's post-hit velocity toward the densest nearby target cluster and add a compression bonus against high-HP or armored targets. |
| Ash | `element_ash` | `A` | Mystic Material | Decay, ember, erosion | Weakens bricks, spreads ruin | Adds decay marks, armor weakening hooks, and ash combo access. | Adds `ash` to `stats.activeElements` and combo detection. If first owned, gives Ash visuals and weakness/resistance lookup. No decay or armor-weakening code yet. | Ash should apply a `decay` status that ticks low damage and temporarily reduces armor; when a decayed target dies, one stack can spread to the nearest damaged target. |
| Glass | `element_glass` | `L` | Mystic Material | Reflection, fracture, prism | Splitting, ricochet, crit shards | Adds shard splitting, ricochet hooks, prism effects, and glass combo access. | Adds `glass` to `stats.activeElements` and combo detection. If first owned, gives Glass visuals, weakness/resistance lookup, and `baseDamageMultiplier: 0.99`. No split, ricochet, or prism code yet. | Glass should create shard hits when it breaks a non-boss target: two low-damage secondary strikes seek nearby targets, with a small pierce chance for the original hit. |
| Mist | `element_mist` | `M` | Mystic Material | Haze, diffusion, concealment | Phasing, soft spread, blur effects | Adds haze fields, phase hooks, soft area damage, and mist combo access. | Adds `mist` to `stats.activeElements` and combo detection. If first owned, gives Mist visuals, weakness/resistance lookup, and `baseDamageMultiplier: 0.98`. No haze, phase, blur, or soft area damage code yet. | Mist should give non-boss hits a modest phase/pierce chance and release a soft haze pulse that deals light damage to nearby already-damaged targets. |
| Iron | `element_iron` | `I` | Mystic Material | Metal, magnetism, force | Armor, attraction, heavy impact | Adds magnetic pull, heavy impact hooks, armor interactions, and iron combo access. | Adds `iron` to `stats.activeElements` and combo detection. If first owned, gives Iron visuals, weakness/resistance lookup, and `baseDamageMultiplier: 1.03`. No magnet, armor, or heavy-impact code yet. | Iron should partially ignore armor on hit and slightly pull the ball toward armored or high-HP targets after collision, making it the reliable heavy-impact element. |
...
- Current code: combos are detection, reveal, save state, and HUD display only. Base Classic element effects can now produce damage-over-time, splash, steering, and tremor behavior, but there is no combo-specific damage, movement, status, board clear, or special reaction hook in `ElementSystem`, `CollisionSystem`, or `BossSystem` yet.
...
- Bespoke gameplay reactions for Spark/Resin/Echo/Gravity/Ash/Glass/Mist/Iron and all named combos are not implemented yet.
- Fire, Water, Wind, and Earth now have baseline hit behavior in addition to visuals, weakness/resistance lookup, combo matching, and HUD display.
